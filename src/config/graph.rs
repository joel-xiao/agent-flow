...
impl GraphConfig {
    ...
    pub fn from_value(value: Value) -> Result<Self> {
        serde_json::from_value(value)
            .map_err(|e| AgentFlowError::Other(anyhow!("Failed to parse graph config value: {}", e)))
    }
    ...
    pub fn get_nodes_by_workflow(&self, workflow_id: &str) -> Vec<&GraphNode> {
        self.nodes.iter()
            .filter(|n| n.workflow.as_ref().map(|w| w == workflow_id).unwrap_or(false))
            .collect()
    }

    /// 获取所有 service 节点
    pub fn get_services(&self) -> Vec<&GraphNode> {
        self.get_nodes_by_type("service")
    }
    ...
    pub fn get_edges_by_workflow(&self, workflow_id: &str) -> Vec<&GraphEdge> {
        self.edges.iter()
            .filter(|e| e.workflow.as_ref().map(|w| w == workflow_id).unwrap_or(false))
            .collect()
    }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        for node in &self.nodes {
            if !node_ids.insert(&node.id) {
                return Err(AgentFlowError::Other(anyhow!("Duplicate node ID: {}", node.id)));
            }
        }
        ...
        for edge in &self.edges {
            if !node_ids.contains(&edge.from) {
                return Err(AgentFlowError::Other(anyhow!("Edge references unknown node: {}", edge.from)));
            }
            if !node_ids.contains(&edge.to) {
                return Err(AgentFlowError::Other(anyhow!("Edge references unknown node: {}", edge.to)));
            }
        }
