...
impl RouteMatcher {
    ...
    pub fn match_route(
        ...
        if let Some(route_payload) = route_payload {
            if let Some(route) = route_payload.get(fields::ROUTE) {
                let branches = self.process_route(
                    route,
                    &route_payload,
                    payload,
                    agent_name,
        ...
        if let Some(default_route) = &self.default_route {
            if self.route_targets.contains(default_route) {
                let branches = self.create_default_route(
                    payload,
                    agent_name,
                    default_route,
                )?;
    ...
    fn process_route(
        ...
        if let Some(route_str) = route.as_str() {
            if let Some(branch) = self.match_single_route(
                route_str,
                route_payload,
                payload,
                agent_name,
        ...
        // 处理多路由标签（数组）
            ...
            let multi_branches = self.match_multiple_routes(
                &route_labels,
                route_payload,
                payload,
                agent_name,
    ...
    fn create_default_route(
        ...
        route_message_payload[fields::ROUTE_REASON] =
            json!(prompt_consts::DEFAULT_ROUTE_REASON);
